    ...
    public function bankSoalDetailTable($id)
    {
        ...
        return Datatables::of($detail)
            ...
            ->addColumn('soal', function ($detail) {
                $html = '';
                $gambar = '<div><a href="' . asset('images/banksoal/') . '/' . $detail->gambar_soal . '" target="_blank"><img style="width:90px;" class="img-responsive" src="' . asset('images/banksoal/') . '/' . $detail->gambar_soal . '"></a><small onclick="deleteImage(' . $detail->id . ', 0)" style="color:red;cursor:pointer;"><i class="fa fa-trash"></i> Delete</small></div>';
                $soal = $detail->soal;
                $soalBawah = $detail->soal_bawah ?? '';
